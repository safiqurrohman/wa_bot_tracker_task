...
client.on('message_create', async (message) => {
    ...
    if (text.startsWith('report') || text.startsWith('laporan')) {
        ...
        if (parts.length === 1) {
            ...
            title = 'Semua Waktu';
            db.query(
                'SELECT status, COUNT(*) as jumlah FROM tasks WHERE user_phone = ? GROUP BY status',
                [user],
            ...
            db.query(
                'SELECT status, COUNT(*) as jumlah FROM tasks WHERE user_phone = ? AND tanggal BETWEEN ? AND ? GROUP BY status',
        ...
        function handleReport(err, results, rangeTitle) {
            ...
            results.forEach(res => {
                if (res.status === 1) selesai = Number(res.jumlah);
                else belum = Number(res.jumlah);
                total += Number(res.jumlah);
            });
